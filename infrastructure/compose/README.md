...
The isolated Booking/Payment topology is `booking-payment.compose.yml`. Booking uses API port `8085` and PostgreSQL port `5437`; Payment uses API port `8084` and PostgreSQL port `5436`. It discovers Account ES256 keys from Account JWKS and requires PayHere sandbox credentials, a public HTTPS callback URL, Event API on port `8082`, and RabbitMQ from the Account topology. Its optional `messaging` profile starts both outbox relays and the Booking Payment consumer. See `docs/runbooks/booking-payment-local.md`.

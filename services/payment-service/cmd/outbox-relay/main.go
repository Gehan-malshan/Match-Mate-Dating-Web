...
func main() {
	...
	for {
		...
		case <-ticker.C:
			...
			if e != nil {
				log.Error("payment_outbox_claim_failed", "error", e)
				continue
			}
			for _, record := range records {
				...
				if e != nil {
					log.Error("payment_outbox_publish_failed", "event_id", record.ID, "error", e)
					break
				}
				select {
				case confirmation := <-confirms:
					if !confirmation.Ack {
						log.Error("payment_outbox_not_acknowledged", "event_id", record.ID)
						continue
					}
				case <-time.After(5 * time.Second):
					log.Error("payment_outbox_confirm_timeout", "event_id", record.ID)
					continue
				case <-ctx.Done():
					return
				}
				if e = repo.MarkOutboxPublished(ctx, record.ID, time.Now().UTC()); e != nil {
					log.Error("payment_outbox_mark_failed", "event_id", record.ID, "error", e)
				}
			}
		}
	}
